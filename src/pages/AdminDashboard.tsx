  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-[#0B0F14] text-white overflow-hidden">
        {/* Header */}
        <header className="bg-[#0D1117] border-b border-gray-800 h-14 flex items-center px-4 sm:px-6 md:px-8 justify-between">
  ...
  if (!isSignedIn || !authUser || authUser.role !== 'admin') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0B0F14]">
        <div className="bg-[#0D1117] p-8 rounded-lg border border-gray-800 text-center">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center">
                <button
                  className="mr-4 lg:hidden text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500"
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h1 className="text-base hidden  sm:text-lg font-semibold text-white">Admin Dashboard</h1>
                    <p className=" sm:text-2xl font-semibold text-white **:sm:block">Administrator Panel</p>
                  </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm font-medium text-white bg-gray-700 rounded-md hover:bg-gray-600 transition-colors duration-200 flex items-center"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-cyan-500 rounded-md hover:from-blue-600 hover:to-cyan-600 transition-colors duration-200 flex items-center"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
        ...
        <main className="flex-1 overflow-y-auto py-6">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-[#0D1117] border border-gray-800 rounded-lg p-6">
              ...
              {activeTab === 'stats' && (
                <div>
                  <h2 className="text-lg font-medium text-white mb-4">Platform Statistics</h2>
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        <div className="bg-[#1E293B] p-4 rounded-lg border border-gray-700">
                          ...
                          <p className="text-2xl font-bold text-blue-300">{stats.users?.total || 0}</p>
                        </div>
                        <div className="bg-[#1E293B] p-4 rounded-lg border border-gray-700">
                          <p className="text-sm text-green-400">Active Users</p>
                          <p className="text-2xl font-bold text-green-300">{stats.users?.active || 0}</p>
                        </div>
                        <div className="bg-[#1E293B] p-4 rounded-lg border border-gray-700">
                          <p className="text-sm text-purple-400">Total Sessions</p>
                          <p className="text-2xl font-bold text-purple-300">{stats.sessions?.total || 0}</p>
                        </div>
                        <div className="bg-[#1E293B] p-4 rounded-lg border border-gray-700">
                          <p className="text-sm text-yellow-400">Active Sessions</p>
                          <p className="text-2xl font-bold text-yellow-300">{stats.sessions?.active || 0}</p>
                        </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="border border-gray-700 rounded-lg p-4 bg-[#0F172A]">
                          <h3 className="font-medium text-white mb-2">User Distribution</h3>
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              <span className="text-gray-300">Admins</span>
                              <span className="text-gray-400">{stats.users?.admins || 0} ({Math.round(((stats.users?.admins || 0) / Math.max(1, stats.users?.total || 1)) * 100)}%)</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-300">Regular Users</span>
                              <span className="text-gray-400">{(stats.users?.total || 0) - (stats.users?.admins || 0)} ({Math.round(((stats.users?.total || 0) - (stats.users?.admins || 0)) / Math.max(1, stats.users?.total || 1) * 100)}%)</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="border border-gray-700 rounded-lg p-4 bg-[#0F172A]">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              <span className="text-gray-300">Messages (30 days): </span>
                              <span className="text-gray-400">{stats.messages?.recent30Days || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-300">Total Snippets: </span>
                              <span className="text-gray-400">{stats.snippets?.total || 0}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="mt-6 border border-gray-700 rounded-lg p-4 bg-[#0F172A]">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-medium text-white">Real-time Activity Monitor</h3>
                      <div className="flex items-center">
                        ...
                        {socketConnectionError && (
                          <span className="text-sm text-red-400 ml-2" title={socketConnectionError}>
                            ⚠️ Connection Issue
                          </span>
                        )}
                      </div>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto bg-[#0B0F14] p-3 rounded border border-gray-800">
                      {adminNotifications.length > 0 ? (
                        adminNotifications.map((notification, index) => (
                          <div key={index} className="flex items-start p-3 bg-[#1E293B] rounded border-l-4 border-blue-500">
